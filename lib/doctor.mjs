...
const EXPECTED_TASKS = {
  frontend: ["reference-analyst", "art-director", "frontend-architect", "interaction-designer", "motion-designer", "threejs-art-director", "responsive-specialist", "visual-critic", "general", "explore"],
...
const LIVE_PAIRS = { "art-director": "threejs-art-director", "interaction-designer": "motion-designer", "responsive-specialist": "visual-critic",
...
async function fleet(repo, live_, live) {
  ...
  for (const a of FLEET) {
    ...
    if (a !== "frontend" && dbg.tools?.edit !== false) p.push("edit not denied");
  ...
  const cyc = FLEET.filter((a) => a !== "frontend" && reach(a).includes("CYCLE")), leak = FLEET.filter((a) => a !== "frontend" && reach(a).includes("general"));
  check(!cyc.length && !leak.length, `delegation graph acyclic=${!cyc.length}, only frontend reaches the writer=${!leak.length}`);
