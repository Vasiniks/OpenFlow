...
const EXPECTED_TASKS = {
  frontend: ["reference-analyst", "art-director", "frontend-architect", "interaction-designer", "motion-designer", "threejs-art-director", "responsive-specialist", "visual-critic", "asset-producer", "general", "explore"],
...
const LIVE_PAIRS = { "asset-producer": null, "art-director": "threejs-art-director", "interaction-designer": "motion-designer", "responsive-specialist": "visual-critic",
...
async function fleet(repo, live_, live) {
  ...
  for (const a of FLEET) {
    ...
    if (!["frontend", "asset-producer"].includes(a) && dbg.tools?.edit !== false) p.push("edit not denied");
  ...
  const cyc = FLEET.filter((a) => a !== "frontend" && reach(a).includes("CYCLE")), leak = FLEET.filter((a) => a !== "frontend" && (reach(a).includes("general") || reach(a).includes("asset-producer")));
  check(!cyc.length && !leak.length, `delegation graph acyclic=${!cyc.length}, only frontend reaches the writers (general, asset-producer)=${!leak.length}`);
...
async function liveTest() {
  ...
  check(!rows.some((r) => r.agent === "general" && r.parent !== "frontend"), `session tree: ${rows.length - 1} delegations, no specialist reached \`general\``);
}
