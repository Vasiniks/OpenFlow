...
export async function doctor({ repo, only, live }) {
  ...
  if (n.FAIL || n.WARN) console.log("Run `openflow fix` to repair what can be repaired automatically.");
  return n.FAIL ? 1 : 0;
}
...
async function fleet(repo, live_, live) {
  ...
  for (const a of FLEET) {
    ...
    // the rtk OpenCode plugin rewrites `ls x` → `rtk ls x` (and `cat f` → `rtk read f`) before subagent permission checks
    if (existsSync(join(CONFIG, "opencode", "plugins", "rtk.ts")) && /"ls \*": allow/.test(fm) && !/"rtk ls \*": allow/.test(fm)) p.push("`ls *` allowed without `rtk ls *` (the rtk plugin rewrites it, so the subagent is denied)");
    if (a === "frontend") { const ed = (dbg.permission || []).filter((x) => x.permission === "edit"); if (!ed.some((x) => x.pattern === "*" && x.action === "deny")) p.push("frontend may edit source (should delegate to builder)"); }
