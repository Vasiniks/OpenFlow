...
const ACCOUNT = /Billing verification failed[^\n]*|payment method[^\n]*|insufficient (credit|quota|balance)[^\n]*|quota exceeded[^\n]*|Unauthorized[^\n]*|invalid api key[^\n]*/i;
...
export async function runResilient({ agent = "frontend", command, args = [], log, retries = 6 }) {
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    ...
    if (ACCOUNT.test(tail)) { warn(`${bold("account/provider problem")}: ${(tail.match(ACCOUNT) || [""])[0]}. Fix it (billing, key or quota, or pick another model with \`opencode models\`), then run the same command again: it resumes from .design/.`); return code; }
