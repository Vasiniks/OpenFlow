...
export async function githubMcp() {
...
export async function motionAi() {
...
// ------------------------------------------------------------------------------------------------ skills
export async function skills(hs, ctx) {
  ...
  for (const [src0, names] of sources) {
    ...
    const local = src0.startsWith("./");  // OpenFlow's own skills change with the repo: always re-sync them
    const need = (dir, skipClaude) => names.filter((n) => !(skipClaude && CLAUDE_VIA_PLUGIN.includes(n)) && (local || !existsSync(join(dir, n, "SKILL.md"))));
