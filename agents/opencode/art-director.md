...
| 3 sources | `skill({ name: "asset-forge" })` + `skill({ name: "asset-library" })` | what can really be produced: CC0 photos, HDRIs, Poly Haven models, headless-Blender product modelling (`forge blender` template), renders, scroll sequences, procedural objects; `forge image` only with an API key (check `echo $OPENAI_API_KEY$FAL_KEY`), otherwise it is moodboard-only — plan photos/renders instead. License rules, effect defaults, avoid list |
